...
export default function InvoiceCard({ invoice, onDelete, hasContract = false }) {
  ...
  // Same label the card header shows, so the confirmation names the invoice the user
  // is looking at rather than an opaque id.
  const label = invoice.invoice_number ? `Invoice ${invoice.invoice_number}` : invoice.file_name;

  const confirmDelete = (e) => {
    e.stopPropagation();
    // Matches the existing convention in ProjectsView.jsx. Delete sits beside the flag
    // badges in the header, so a misclick permanently removed an invoice and silently
    // re-flagged the rest.
    if (!window.confirm(`Delete ${label}? The billing sheet will be recalculated without it.`)) return;
    onDelete(invoice.id);
  };
  ...
  return (
    <div className="invoice-card">
      <div className="invoice-card-header" onClick={() => setOpen((o) => !o)}>
        <div>
          <div className="invoice-card-title">{label}</div>
        ...
        <div className="invoice-card-flags">
          {invoice.verification_status && invoice.verification_status !== "not_asserted" && (
            <span
              className={`badge-severity ${
                invoice.verification_status === "rejected"
                  ? "critical"
                  : invoice.verification_status === "review"
                  ? "warning"
                  : "info"
              }`}
              title={`Double-LLM verification of the numbers read from this PDF${
                invoice.verification_trust != null ? ` (${invoice.verification_trust}% grounded in the source)` : ""
              }`}
            >
              {invoice.verification_status === "verified"
                ? `✓ Verified${invoice.verification_trust != null ? ` ${invoice.verification_trust}%` : ""}`
                : invoice.verification_status === "review"
                ? "⚠ Verify numbers"
          ...
          <button type="button" className="btn btn-secondary btn-sm" onClick={confirmDelete}>
            Delete
          </button>
        </div>
      </div>
            ...
            <div style={{ marginBottom: "var(--fs-space-2)" }}>
              {sortedFlags.map((f) => (
                <div key={f.id} className="flag-row">
                  ...
                  <span>{f.message}</span>
                  <Feedback agent="invoice" targetType="invoice_flag" targetRef={`${f.rule_code}:${f.id}`} />
                </div>
              ))}
            </div>
          )}
