      ...
      <div className="chat-messages" ref={scrollRef}>
        {messages.length === 0 && (
          <p className="chat-empty">
            Ask anything about this contract or invoices. For example, "How is task 22 trending?" or "Summarize the
          ...
          <div key={m.id} className={`chat-message ${m.role}`}>
            ...
            {m.role === "assistant" && typeof m.id === "number" && (
              <div className="chat-fb">
                <Feedback agent="invoice" targetType="chat_reply" targetRef={String(m.id)} />
              </div>
            )}
          </div>
        ))}
